...
def get_model(model_config: Dict[str, Any]) -> BaseLanguageModel:
    ...
    if model_type == "ollama":
        ...
        print(f"      - Google GenAI mode using ChatGoogleGenerativeAI.")
        
        # 準備要傳遞給模型的額外參數
        model_kwargs = {}
        if model_config.get("reasoning_effort"):
            effort_level = model_config["reasoning_effort"]
            model_kwargs["reasoning_effort"] = effort_level
            print(f"      - Applying Google GenAI options: reasoning_effort='{effort_level}'")

        # 函式庫會自動從環境變數 GOOGLE_API_KEY 讀取金鑰
...
for config_details in MODEL_CONFIGURATIONS.values():
    for model_key, model_info in config_details.items():
        if model_key != "templates":
            # 為 google_genai 類型補上 None 以維持 tuple 結構一致
            key_tuple = (
                model_info["type"], 
    ...
    async def stream_generator() -> AsyncGenerator[str, None]:
        ...
        async def run_analysis(config_name: str, config_details: Dict):
            ...
            try:
                ...
                print(error_message)
                logging.exception(f"Exception details for '{config_name}':")
...
async def query_components_multi_stream_endpoint(request: QueryRequest):
    print("\n" + "="*50)
    print(f"RECEIVED RAG REQUEST:")
    print(f"  - Selected Models: {request.selected_models}")
    print(f"  - User Input (ZH): {request.user_input_zh}")
    print("="*50 + "\n")

    async def stream_generator() -> AsyncGenerator[str, None]:
        queue = asyncio.Queue()
        PIPELINE_STEPS = ["翻譯使用者問題", "分析查詢意圖(RAG)", "路由查詢(選擇模板)", "檢索相關文件", "生成 RAG 回答", "翻譯最終結果", "更新對話摘要"]
        ...
        async def run_full_rag_and_summary_pipeline(config_name: str, config_details: Dict):
            ...
            try:
                ...
                try:
                    ...
                    print(f"[{config_name}] Query analysis for retrieval failed: {e}. Using original query.")
                    await send_status(1, "error", error_msg="意圖分析失敗，使用原始查詢")

                # Step 2: Route query
                await send_status(2, "running")
                prompt_name = "structured_rag"
                try:
                    router_chain = QUERY_ROUTER_PROMPT | rag_llm | JsonOutputParser()
                    routing_decision = await asyncio.wait_for(
                        router_chain.ainvoke({"chat_history": previous_summary, "question": user_input_en}), 
                        timeout=20.0
                    )
                    prompt_name = routing_decision.get("prompt_name", "structured_rag")
                    print(f"[{config_name}] Query Router decided to use: '{prompt_name}'")
                    await send_status(2, "complete")
                except Exception as e:
                    print(f"[{config_name}] Query Router failed: {e}. Defaulting to 'structured_rag'.")
                    await send_status(2, "error", error_msg="路由判斷失敗")
                
                rag_prompt_template = RAG_PROMPTS.get(prompt_name, RAG_PROMPTS["structured_rag"])
                ...
                rag_chain = rag_prompt_template | rag_llm | StrOutputParser()
        ...
        if tasks:
            ...
            while finished_tasks_count < len(tasks):
                data = await queue.get()
                yield f"data: {json.dumps(data, ensure_ascii=False)}\n\n"
                # A full response is sent before the summary, so we wait for summary/error to count a task as "finished"
