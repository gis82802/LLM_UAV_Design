...
# --- Prompt 0: Query Intent Analysis (For Search Scoping) ---
_QUERY_ANALYSIS_TEMPLATE_EN = """
# [Role]
You are a highly efficient query routing engine. Your task is to analyze a user's query and extract key information to narrow down a search.
# [Task]
Analyze the "User Question" to extract two key pieces of information:
1.  `component_types`: Identify and list ALL mentioned component categories from the "Available Categories List". If the user asks a general question without specifying any type, return an empty list `[]`.
2.  `keywords`: Generate a detailed string of search keywords based on the user's question and the overall mission context.
# [Execution Rules]
- Your response MUST be a correctly formatted JSON object.
- `component_types` MUST be a list of strings.
- `keywords` should be rich and descriptive for better search results.
# [Input Data]
---
### Mission Summary
{mission_summary}
...
---
# [Example Output]
User Question: "Which batteries perform well in low temperatures?" -> {{"component_types": ["batteries"], "keywords": "low temperature high performance batteries for long flight endurance drone"}}
User Question: "Recommend some components." -> {{"component_types": [], "keywords": "suitable components for mission"}}
"""
QUERY_ANALYSIS_PROMPT = PromptTemplate.from_template(_QUERY_ANALYSIS_TEMPLATE_EN)


# --- Prompt 1: Initial Mission Analysis (English Version) ---
...
# --- Prompt 2: Unified RAG Prompt with Conditional Output Logic ---
_UNIFIED_RAG_PROMPT_STR = """
# Role
...
# Your Response
"""
UNIFIED_RAG_PROMPT = ChatPromptTemplate.from_template(_UNIFIED_RAG_PROMPT_STR)
