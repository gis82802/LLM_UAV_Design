...
This chapter describes the ZED-F9K operational features and their configuration.
...
<span id="page-9-4"></span><sup>1</sup> For B2a navigation data (B-CNAV1 and B-CNAV2) signal-in-space (SiSA) accuracy values have not been defined in the ICD yet
...
ZED-F9K supports SPARTN as specified by SPARTN Interface Control Document – Version 2.0.2 (February, 2022).
...
Only one source can be configured to be used at a time by ZED-F9K. The configuration item CFG-SPARTN-USE\_SOURCE can be configured to select which source will be used. Alternatively, the input protocol configuration items of a physical port can be configured to block input support for the UBX or the SPARTN protocols on the desired ports, for example CFG-UART1INPROT-UBX, CFG-USBINPROT-SPARTN, etc.
...
With the LAP 1.30 firmware onwards, the wake on motion feature supports the host only option, i.e. CFG-HW-SENS\_WOM\_MODE should be set to 1. Waking up the receiver is currently not supported
...
# <span id="page-45-0"></span>**3.3 Primary and secondary output**

Supported as of firmware version LAP 1.30

# <span id="page-45-1"></span>**3.3.1 Introduction**
...
The ZED-F9K can provide this output in two streams:

- **Primary output:** Reports the results of a full navigation solution using all capabilities of the ZED-F9K, such as, for example, lane accurate sensor fusion positioning.
- **Secondary output:** Reports the results of a GNSS standalone navigation solution.

Both the primary output and secondary output provide a similar set of information but the two outputs report different results.The primary output is reported in the form of UBX-NAV-\* messages, while the secondary output is reported in the form of UBX-NAV2-\* messages. Therefore, the UBX message class can be used to distinguish between the primary output and the secondary output.



For the specification of the UBX-NAV2-\* messages and for a full list of available UBX-NAV2-\* messages, see the applicable Interface description [[2](#page-126-2)].

- The secondary output may also be available as NMEA-NAV2-\* messages. To confirm availability of NMEA-NAV2-\* messages, see the applicable Interface description [[2](#page-126-2)]. NMEA-NAV2-\* messages can only be output if the configured NMEA version is NMEA 4.0 or later. For simplicity this section refers only to UBX-NAV2-\* messages but the same information applies for NMEA-NAV2-\* messages.
- The secondary output is complementary to the primary output. It does not provide the full navigation solution of the primary output. It can be used to expand the applications of the ZED-F9K to enable using a second navigation solution in parallel with the primary navigation solution.
...
# <span id="page-46-0"></span>**3.3.2 Configuration**
...
Due to the increased message output, the interface load will be higher while the secondary output messages are enabled. Therefore, the interface baud rate may need to be adapted



accordingly. Alternatively, it is possible to configure the UBX-NAV2-\* messages with a different output rate from that of their primary output UBX-NAV-\* counterparts.
...
A minimal subset of the primary output navigation solution configuration is available for the secondary output navigation solution configuration. All such available configuration items are in the CFG-NAV2-\* configuration group (see applicable Interface description [\[2](#page-126-2)]).
...
# <span id="page-47-0"></span>**3.3.3 Expected output behavior**

Once the secondary output is enabled and the desired secondary output UBX-NAV2-\* messages are configured, the ZED-F9K will output both primary and secondary output data in the form of the enabled UBX-NAV-\* and UBX-NAV2-\* messages respectively.
...
The specification of the UBX-NAV2-\* messages resembles that of the UBX-NAV-\* messages. The payload specification of a UBX-NAV2 message is identical to the payload specification of its UBX-NAV counterpart, allowing to easily adapt any existing message parsers. The primary output will contain results and data reflecting the full navigation solution of the ZED-F9K. The secondary output will contain results and data reporting a GNSS standalone navigation solution.

# <span id="page-47-1"></span>**3.3.4 Example use cases**

As an example, an application using a ZED-F9K that is operating in RTK sensor fusion mode can enable the secondary output and monitor a GNSS standalone solution to understand the improvements introduced by using lane accurate sensor fusion.



# <span id="page-48-0"></span>**3.4 SBAS**

The ZED-F9K is capable of receiving multiple SBAS signals concurrently, even from different SBAS systems (WAAS, EGNOS, MSAS, etc.). They can be tracked and used for navigation simultaneously. Every SBAS satellite that broadcasts ephemeris or almanac information can be used for navigation, just like a normal GNSS satellite.

For receiving correction data, the ZED-F9K automatically chooses the best SBAS satellite as its primary source. It will select only one since the information received from other SBAS satellites is redundant and could be inconsistent. The selection strategy is determined by the proximity of the satellites, the services offered by the satellite, the configuration of the receiver (test mode allowed/ disallowed, integrity enabled/disabled) and the signal link quality to the satellite.

If corrections are available fromthe chosenSBASsatellite and used in the navigation calculation, the differential status will be indicated in several output messages such as UBX-NAV-PVT, UBX-NAV-STATUS, UBX-NAV-SAT, NMEA-GGA, NMEA-GLL, NMEA-RMC and NMEA-GNS (see the applicable Interface description [\[2\]](#page-126-2)). The message UBX-NAV-SBAS provides detailed information about which corrections are available and applied.
...
# <span id="page-49-0"></span>**3.5 BeiDou SBAS configuration**
...
The CFG-SBAS-PRNSCANMASK in[Table](#page-49-1) 21 includes onlyBeiDouSBASPRNs. Alternatively, the BeiDou SBAS PRNs can be added to the default list of PRNs enabled in the firmware or modified list of PRNs that is configured. This allows other SBAS systems to be used if BeiDou SBAS is not available.

# <span id="page-50-0"></span>**3.6 QZSS SLAS**
...
# <span id="page-50-2"></span>**3.6.2 Configuration**
...
# <span id="page-51-0"></span>**3.7 Protection level**

Supported as of firmware version LAP 1.30

# <span id="page-51-1"></span>**3.7.1 Introduction**
...
# <span id="page-51-2"></span>**3.7.2 Interface**

The protection level bounds the true position error with a target misleading information risk (TMIR), for example 5[%MI/epoch] (read: 5% probability of having an MI per epoch). The target misleading information risk describes the probability per epoch of having misleading information (MI), that is, the true position error is larger than the protection level and fails to bound the true position error (see [Figure](#page-52-0) 11).
...
- Protection level computing can be disabled through the CFG-NAVSPG-PL\_ENA configuration item. See the applicable interface description [\[2\]](#page-126-2) for availability of the configuration item.
...
[Table](#page-53-1) 25 lists the requirements for using the PL feature.
...
# <span id="page-54-1"></span>**3.8 Communication interfaces**

u-blox receivers are equipped with a communication interface [7](#page-54-2) which is multi-protocol capable. The interface ports can be used to transmit GNSS measurements, monitor status information and configure the receiver.
...
The ZED-F9K provides UART1, UART2, SPI, I2C and USB interfaces for communication with a host CPU. The interfaces are configured via the configuration methods described in the applicable interface description [\[2\]](#page-126-2).
...
<span id="page-54-2"></span>7 The signal names and related terms have been replaced with new terminology in this document.
...
**Figure 13: ZED-F9K output isolation**



**Figure 14: ZED-F9K input isolation**

# <span id="page-55-0"></span>**3.8.1 UART**
...
# <span id="page-56-0"></span>**3.8.2 I2C interface**
...
In designs where the host uses the same I2C bus to communicate with more than one u-blox receiver, the I2C peripheral address for each receiver must be configured to a different value. Typically most u-blox receivers are configured to the same default I2C peripheral address value. To poll or set the I2C peripheral address, use the CFG-I2C-ADDRESS configuration item (see the applicable interface description [[2](#page-126-2)]).
...
The I2C interface allows 256 registers to be addressed. As shown in [Figure](#page-57-0) 15, only three of these are currently implemented.
...
The receiver does not provide any write access except for writing UBX and NMEA messages to the receiver, such as configuration or aiding data. Therefore, the register set mentioned in the section Read [access](#page-59-1) is not writeable.
...
# <span id="page-59-0"></span>**3.8.3 SPI interface**

ZED-F9K has an SPI peripheral interface that can be selected by setting D\_SEL = 0. The SPI peripheral interface is shared with UART1 and I2C port, the physical pins are same. The SPI pins available are:
...
See more information about communication interface selection from the [D\\_SEL](#page-61-1) section.
...
### <span id="page-59-1"></span>**3.8.3.1 Read access**

As the register mode is not implemented for the SPI port, only the UBX/NMEA message stream is provided. This stream is accessed using the back-to-back read and write access (see section [Back](#page-59-2)[to-back](#page-59-2) read and write access below). When no data is available to be written to the receiver, SDI should be held logic high, i.e. all bytes written to the receiver are set to 0xFF.
...
#### <span id="page-59-2"></span>**3.8.3.2 Back-to-back read and write access**
...
# <span id="page-60-0"></span>**3.8.4 USB interface**
...
The ZED-F9K receiver supports only self-powered mode operation in which the receiver is supplied from its own power supply. The V\_USB pin is used to detect the availability of the USB port, i.e. whether the receiver is connected to a USB host.
...
**Figure 20: ZED-F9K example circuit for USB interface**

R11 = 100 k Ω is recommended

R4, R5 = 27 Ω is recommended

# <span id="page-61-0"></span>**3.9 Predefined PIOs**

In addition to the communication ports, there are some predefined PIOs provided by ZED-F9K to interact with the receiver. These PIOs are described in this section.

If hardware backup mode is used a proper isolation of the interfaces is needed.

# <span id="page-61-1"></span>**3.9.1 D\_SEL**

The D\_SEL pin can be used to configure the functionality of the combined UART1, I2C, and SPI pins. It is possible to configure the pins as UART1 + I2C, or as SPI. SPI is not available unless D\_SEL pin is set to low. See [Table](#page-61-4) 29 below.
...
# <span id="page-61-2"></span>**3.9.2 RESET\_N**

The ZED-F9K provides the ability to reset the receiver. The RESET\_N pin is an input-only pin with an internal pull-up resistor. Driving RESET\_N low for at least 100 ms will trigger a cold start.

The RESET\_N pin will delete all information and trigger a cold start. It should only be used as a recovery option.

# <span id="page-61-3"></span>**3.9.3 SAFEBOOT\_N**

The ZED-F9K provides a SAFEBOOT\_N pin that is used to command the receiver safeboot mode.
...
# <span id="page-62-0"></span>**3.9.4 TIMEPULSE**

The ZED-F9K high precision receiver provides a time pulse on the TIMEPULSE pin.

More information about the time pulse feature and its configuration can be found in the Time [pulse](#page-74-0) section.

# <span id="page-62-1"></span>**3.9.5 TX\_READY**
...
# <span id="page-62-2"></span>**3.9.6 EXTINT**
...
# <span id="page-63-0"></span>**3.9.7 WT and DIR inputs**
...
By default, the DIR pin polarity is automatically initialized once the vehicle has reached the required minimum speed (see Accelerated [initialization](#page-36-0) and calibration procedure section). The DIR pin polarity can also be configured with the CFG-SFODO-DIR\_PINPOL key.
...
For more information, see the interface description [\[2](#page-126-2)].

# <span id="page-63-1"></span>**3.9.8 RTK\_STAT interface**

The ZED-F9K provides an RTK\_STAT pin that provides an indication of the RTK positioning status. It can be used to confirm if a valid stream of correction messages is being received. As valid correction messages we only consider the correction messages that are supported and used by the receiver.
...
# <span id="page-64-0"></span>**3.10 Antenna supervisor**
...
The Antenna [supervisor](#page-101-0) circuit section details the required circuit and the following sections explain how to enable and monitor each feature:

# <span id="page-64-1"></span>**3.10.1 Antenna voltage control - ANT\_OFF**
...
# <span id="page-64-2"></span>**3.10.2 Antenna short detection - ANT\_SHORT\_N**
...
# <span id="page-65-0"></span>**3.10.3 Antenna short detection auto recovery**
...
# <span id="page-65-1"></span>**3.10.4 Antenna open circuit detection - ANT\_DETECT**
...
# <span id="page-66-0"></span>**3.11 Multiple GNSS assistance (MGA)**
...
The ZED-F9K supports AssistNow Online only.

# <span id="page-66-1"></span>**3.11.1 Authorization**
...
# <span id="page-66-2"></span>**3.11.2 Preserving MGA and operational data during power-off**
...
See section [V\\_BCKP:](#page-94-2) Backup supply voltage for more information.

- **Save-on-shutdown:**The receiver can be instructed to dump its current state to flash memory as part of the shutdown procedure; this data is then automatically retrieved when the receiver is restarted. For more information, see section [Save-on-shutdown](#page-67-0) feature for more info. For information on the UBX-UPD-SOS messages consult the applicable Interface description [[2\]](#page-126-2).
- **Advanced calibration handling:**The receiver can be instructed to dump its current state (UBX-MGA-SF and UBX-NAV-PVAT) to host as part of the shutdown procedure or during the drive; this data is then sent back to the receiver when the receiver is restarted. For more information, see section Advanced [calibration](#page-68-0) handling for more info.
- **Database dump:** The receiver can be made to dump the state of its navigation database in the form of a sequence of UBX messages reported to the host; these messages can be stored by the host and then sent back to the receiver when it has been restarted. See the description of the UBX-MGA-DBD messages in the applicable Interface description [\[2\]](#page-126-2) for more information.

# <span id="page-67-0"></span>**3.11.3 Save-on-shutdown feature**
...
# <span id="page-68-0"></span>**3.11.4 Advanced calibration handling**

Supported from firmware version LAP 1.30 onwards
...
**Figure 21: Advanced calibration handling operational steps**

# <span id="page-69-0"></span>**3.12 Clocks and time**

This section introduces and explains the concepts of receiver clocks and time bases.

# <span id="page-69-1"></span>**3.12.1 Receiver local time**
...
# <span id="page-70-0"></span>**3.12.2 Navigation epochs**
...
u-blox receivers employ multiple GNSS system times and/or receiver local times (in order to support multiple GNSSsystems concurrently), so users should not use UBX messages reporting



GNSS system time or receiver local time. It is recommended to use messages that report UTC time and other messages are retained only for backwards compatibility reasons.

# <span id="page-71-0"></span>**3.12.3 iTOW timestamps**
...
With Priority [navigation](#page-42-0) mode enabled, iTOW can be used as an epoch collector only for messages of the same group (e.g. priority navigation messages with the same iTOW value can be assumed to have come from the same navigation solution).
...
If reliable absolute time information is required, users are recommended to use the UBX-NAV-PVT navigation solution message which also contains additional fields that indicate the validity (and accuracy in UBX-NAV-PVT) of the calculated times (see also the [GNSS times](#page-71-1) section below for further messages containing time information).

# <span id="page-71-1"></span>**3.12.4 GNSS times**
...
# <span id="page-71-2"></span>**3.12.5 Time validity**
...
# <span id="page-72-0"></span>**3.12.6 UTC representation**
...
# <span id="page-72-1"></span>**3.12.7 Leap seconds**

Occasionally it is decided (by one of the international time keeping bodies) that, due to the slightly uneven spin rate of the Earth, UTC has moved sufficiently out of alignment with mean solar time (i.e.



the Sun no longer appears directly overhead at 0 longitude at midday). A "leap second" is therefore announced to bring UTC back into close alignment. This normally involves adding an extra second to the last minute of the year, but it can also happen on 30th June. When this happens UTC clocks are expected to go from 23:59:59 to 23:59:60 and only then on to 00:00:00.
...
# <span id="page-73-0"></span>**3.12.8 Real-time clock**
...
# <span id="page-73-1"></span>**3.12.9 Date**
...
# <span id="page-74-0"></span>**3.12.10 Time pulse**

# **3.12.10.1 Introduction**
...
#### **Figure 23: Time pulse and TIM-TP**

## <span id="page-75-0"></span>**3.12.10.3 GNSS time bases**
...
u-blox receivers allow users to independently choose GNSS signals used in the receiver (using CFG-SIGNAL-\*) and the input/output time base (using CFG-TP-\*). For example, it is possible to instruct the receiver to use GPS and Galileo satellite signals to generate BeiDou time.



This practice will compromise time pulse accuracy if the receiver cannot measure the timing difference between the constellations directly and is therefore not recommended.
...
### **3.12.10.4 Time pulse configuration**

u-blox ZED-F9K receivers provide a time pulse (TIMEPULSE) signal with a configurable pulse period, length and polarity (rising or falling edge).
...
# <span id="page-76-0"></span>**3.12.10.5 Configuring time pulse with CFG-TP-\***
...
# <span id="page-77-0"></span>**3.12.11 Time mark**
...
Only the last rising and falling edge detected between two epochs is reported since the output rate of the UBX-TIM-TM2 message corresponds to the measurement rate configured with CFG-RATE-MEAS (see [Figure](#page-78-1) 25 below).
...
# <span id="page-78-0"></span>**3.13 Security**

The security concept of ZED-F9K covers:
...
# <span id="page-79-0"></span>**3.13.1 Spoofing detection and monitoring**
...
Furthermore, with LAP 1.50, the receiver supports sensor-based spoofing detection. If the sensors are initialized, the receiver detects the presence of spoofing by comparing GNSS information against the sensor data.
...
# <span id="page-79-1"></span>**3.13.2 Jamming and interference detection and monitoring**
...
# <span id="page-79-2"></span>**3.13.3 Spoofing and jamming indication**
...
See the applicable Interface description [\[2\]](#page-126-2).

# <span id="page-79-3"></span>**3.13.4 GNSS receiver security**

#### <span id="page-79-4"></span>**3.13.4.1 Secure boot**

The ZED-F9K boots onlywithfirmware images that are signed by u-blox.This prevents the execution of non-genuine firmware images on the receiver.



# <span id="page-80-2"></span>**3.13.4.2 Secure firmware update**

The firmware image is signed by u-blox. The ZED-F9K verifies the signature during the firmware update.

#### <span id="page-80-3"></span>**3.13.4.3 Receiver configuration lock**
...
For more information on the configuration layers including the order of priority they are applied in, see the applicable interface description [[2](#page-126-2)].

The configuration lock set on a configuration layer in volatile memory (RAM, BBR) is removed when the memory is cleared. However, the configuration lock set in non-volatile memory (flash memory) is permanent apart from one exception: during firmware upload to flash memory, the flash is erased during the process causing the configuration lock to be cleared. Refer to [Firmware](#page-89-2) upload for more information on firmware update.
...
An example of use case is that the host application locks the receiver configuration. A user communicating with the ZED-F9K through any of the available interfaces can poll, enable or send messages, but cannot change the configuration by sending UBX configuration messages.

# <span id="page-80-0"></span>**3.14 u-blox protocol feature descriptions**

# <span id="page-80-1"></span>**3.14.1 Broadcast navigation data**
...
#### **3.14.1.1 Parsing navigation data subframes**
...
#### **Figure 28: GPS L5 subframe words**
...
#### **Figure 29: GLONASS navigation message data**
...
| <b>MSB</b>                |                 | <b>LSB</b>       |
|---------------------------|-----------------|------------------|
| Pad<br>to<br>2 bits<br>10 | Data<br>22 bits | Parity<br>8 bits |

#### **Figure 30: BeiDou subframe word**
...
**Figure 31: Galileo E1-B subframe words**
...
**Figure 32: Galileo E5b subframe words**



# **3.14.1.5.3 Galileo E5a**
...
#### **Figure 33: Galileo E5a subframe words**
...
# <span id="page-89-0"></span>**3.15 Forcing a receiver reset**
...
# <span id="page-89-1"></span>**3.16 Firmware**

# <span id="page-89-2"></span>**3.16.1 Firmware upload**
...
[Figure](#page-90-1) 35 shows the spectrum view in u-center with the view/hold option selected. The red line represents the frozen spectrum before modifying the external gain, while the black line represents the current measurement.

<span id="page-90-1"></span>

**Figure 35: Spectrum analyzer view in u-center with the option view/hold selected**



- The span frequency depends on the number of constellations enabled which impacts the spectrum resolution owing to a fixed set of points. For further details about this message see the Interface description [\[2\]](#page-126-2).
...
# <span id="page-92-0"></span>**4 Design**

This section provides information to help carry out a successful schematic and PCB design integrating the ZED-F9K.

# <span id="page-92-1"></span>**4.1 Pin assignment**

The pin assignment of the ZED-F9K module is shown in [Figure](#page-92-2) 36. The defined configuration of the PIOs is listed in [Table](#page-92-3) 33.

ZED-F9K is an LGA package with the I/O on the outside edge and central ground pads.

<span id="page-92-2"></span>

#### **Figure 36: ZED-F9K pin assignment**
...
**Table 33: ZED-F9K pin assignment**

# <span id="page-94-0"></span>**4.2 Power supply**

The u-blox ZED-F9K module has three power supply pins: VCC, V\_BCKP and V\_USB.

# <span id="page-94-1"></span>**4.2.1 VCC: Main supply voltage**
...
- When switching from backup mode to normal operation or at startup, u-blox ZED-F9K modules must charge the internal capacitors in the core domain. In certain situations, this can result in a significant current draw. For low-power applications using backup mode, it is important that the power supply or low ESR capacitors at the module input can deliver this current/charge.
...
# <span id="page-94-2"></span>**4.2.2 V\_BCKP: Backup supply voltage**
...
If a host is connected to ZED-F9K, V\_BCKP can be partially emulated by using UBX-UPD-SOS functionality. BBR data can saved to the host and restored at startup. See the applicable [Interface](#page-126-2) [description](#page-126-2) for more information.
...
- Allow all I/O including UART and other interfaces to float or connect to a high impedance in HW backup mode (V\_BCKP supplied when VCC is removed). See the [Interfaces](#page-61-0) section.
...
# <span id="page-95-0"></span>**4.2.3 ZED-F9K power supply**
...
**Figure 37: ZED-F9K power supply**

# <span id="page-95-1"></span>**4.3 ZED-F9K minimal design**

The minimal electrical circuit for ZED-F9K operation using the UART1 interface is shown in [Figure](#page-96-1) [38](#page-96-1) below.



<span id="page-96-1"></span>

#### **Figure 38: Minimal ZED-F9K design**
...
# <span id="page-96-0"></span>**4.4 WT and DIR interface example**

This section shows an example design for interfacing the WT and DIR pins.



**Figure 39: Example WT and DIR design**
...
# <span id="page-97-0"></span>**4.5 Antenna**

The ZED-F9K requires an active antenna with an integrated LNA to ensure good performance under nominal signal reception.
...
**Figure 40: u-blox low cost dual-band antenna internal structure**
...
|------------------------------------|---------------------------------------------------------------------------------------|-------------------------------|
|                                    | Minimum gain8                                                                         | 17 dB                         |
| Active antenna recommendations     | 8<br>Maximum gain                                                                     | 50 dB                         |
|                                    | Noise figure                                                                          | <4 dB                         |
| 9<br>Group delay variation in-band | 10 ns max at each GNSS system bandwidth.<br>Note: Inter-signal requirement 50 ns max. |                               |
| Out-of-band rejection              | 40 dB typ                                                                             |                               |
|                                    | 10<br>L1 band antenna gain                                                            | 1559 - 1606 MHz: 3 dBic typ.  |
|                                    | 10<br>L2/E5b band antenna gain                                                        | 1197 - 1249 MHz: 2 dBic typ.  |
| Antenna element specification      | 10<br>L5/E5a band antenna gain                                                        | 1164 - 1188 MHz: 2 dBic typ.  |
|                                    | Polarization                                                                          | RHCP                          |
|                                    | Axial ratio                                                                           | 2 dB max, at Zenith           |
|                                    | Phase center variation                                                                | <10 mm over elevation/azimuth |
| 11<br>EMI immunity out-of-band     | 30 V/m                                                                                |                               |
| ESD circuit protection             | 15 kV human body model air discharge                                                  |                               |

**Table 35: Antenna specifications for ZED-F9K modules**
...
# <span id="page-98-0"></span>**4.5.1 Active Antenna Power Supply**

The antenna power supply is typically used to power GNSS active antennas. The power supply should be able to provide the correct voltage and current to the antenna to ensure optimal performance of ZED-F9K.

To power and limit the current to the antenna, you have the following options:

- [External](#page-99-0) power supply
- [External](#page-100-0) power supply and current limiting
- [VCC\\_RF](#page-101-1) power supply
...
<span id="page-98-1"></span><sup>8</sup> Including passive losses (filters, cables, connectors etc.)

<span id="page-98-2"></span><sup>9</sup> GNSS system bandwidths: B1I 1559… 1563 MHz; L1,E1,B1C 1573… 1578 MHz; L1OF 1598… 1606 MHz; E5b,B2I 1192… 1212 MHz; L2C 1223… 1231 MHz; L2OF 1242… 1249 MHz

<span id="page-98-3"></span><sup>10</sup> Measured with a ground plane d=150 mm

<span id="page-98-4"></span><sup>11</sup> Exception L1,L2 ,L5 bands +/- 200 MHz, emphasis on cellular bands
...
**Table 36: ZED-F9K external voltage antenna bias components**

#### <span id="page-100-0"></span>**4.5.1.2 External power supply and current limiting**

[Figure](#page-100-1) 43 shows an example with an external voltage V\_ANT 3.3 V. In this example, the current limiting threshold is set at 60 mA and the use of ferrite bead is recommended.
...
#### **Figure 43: ZED-F9K with external voltage antenna bias and current limit circuit**
...
**Table 37: ZED-F9K antenna bias components**



# <span id="page-101-1"></span>**4.5.1.3 VCC\_RF power supply**

When using the VCC\_RF supply pin from ZED-F9K:
...
**Figure 44: ZED-F9K VCC\_RF antenna bias**
...
**Table 38: ZED-F9K VCC\_RF antenna bias components**

# <span id="page-101-0"></span>**4.5.2 Antenna supervisor circuit**

The active antenna supervisor circuit connects to three ZED-F9K pins:
...
#### **Figure 45: ZED-F9K antenna supervisor circuit**

The bias-T inductor L4 should support multi-band operation within the 1–1.8 GHz frequency range. For additional information, see Active [Antenna](#page-98-0) Power Supply section.

| Specifications                                                          |  |  |
|-------------------------------------------------------------------------|--|--|
| Filtering capacitor                                                     |  |  |
| Minimum Current of 300 mA or more. Impedance >500 Ω at GNSS frequencies |  |  |
| Passive pull-up to control T5                                           |  |  |
| Current limiter in the event of a short circuit                         |  |  |
| Defines the threshold of the comparator                                 |  |  |
| Defines the threshold of the comparator                                 |  |  |
| P-FET transistor acting as a switch to control the antenna supply       |  |  |
| Open drain buffer to shift voltage levels                               |  |  |
| Comparator (op-amp)                                                     |  |  |
|                                                                         |  |  |
...
# <span id="page-103-0"></span>**4.6 EOS/ESD precautions**
...
# <span id="page-103-1"></span>**4.6.1 ESD protection measures**
...
# <span id="page-103-2"></span>**4.6.2 EOS precautions**
...
**Figure 47: Active antenna EOS protection**

# <span id="page-104-0"></span>**4.6.3 Safety precautions**

The ZED-F9K must be supplied by an external limited power source in compliance with the clause 2.5 of the standard IEC 60950-1. In addition to external limited power source, only Separated or Safety Extra-Low Voltage (SELV) circuits are to be connected to the module including interfaces and antennas.

For more information about SELV circuits see section 2.2 in Safety standard IEC 60950-1.

# <span id="page-104-1"></span>**4.7 Electromagnetic interference on I/O lines**
...
# <span id="page-104-2"></span>**4.7.1 General notes on interference issues**
...
# <span id="page-105-0"></span>**4.7.2 In-band interference mitigation**
...
# <span id="page-105-1"></span>**4.7.3 Out-of-band interference**
...
# <span id="page-106-3"></span>**4.8.3 Package footprint, copper and paste mask**

This section provides recommendations for copper and solder mask dimensioning for the ZED-F9K module packages.
...
#### **Figure 48: ZED-F9K suggested footprint (i.e. copper mask)**
...
**Table 41: ZED-F9K footprint dimensions**
...
#### **Figure 49: ZED-F9K suggested paste mask**
...
**Table 42: ZED-F9K paste mask dimensions**

# <span id="page-108-0"></span>**4.8.4 Layout guidance**
...
The RF trace must be shielded by vias to ground along the entire length of the trace and the ZED-F9K RF\_IN pad should be surrounded by vias as shown in the figure below.



**Figure 50: RF input trace**
...
**Figure 51: Top layer fill and vias**



# **4.8.4.3 VCC pads**

The VCC pads for the ZED-F9K high precision receiver must have as low impedance as possible with large vias to the lower power layer of the PCB. The VCC pads need a large combined pad and the decoupling capacitors must be placed as close as possible. This is shown in the figure below.



**Figure 52: VCC pads**

# <span id="page-110-0"></span>**4.9 Design guidance**

# <span id="page-110-1"></span>**4.9.1 General considerations**
...
- Compare the peak current consumption of the ZED-F9K GNSS module with the specification of your power supply.
- GNSS receivers require a stable power supply. Avoid series resistance (less than 0.2 Ω) in your power supply line (the line to VCC) to minimize the voltage ripple on VCC. See the ZED-F9K Power [supply](#page-94-0) section in the [Design](#page-92-0) chapter for more information on the power supply requirements.
- Allow all I/O to Float/High impedance (High-Z) when VCC is not applied.

# <span id="page-110-2"></span>**4.9.2 Backup battery**
...
# <span id="page-111-0"></span>**4.9.3 RF front-end circuit options**
...
# <span id="page-111-1"></span>**4.9.4 Antenna/RF input**
...
# <span id="page-112-0"></span>**4.9.5 Ground pads**

Ensure the ground pads of the module are connected to ground.

# <span id="page-112-1"></span>**4.9.6 Schematic design**
...
# <span id="page-112-2"></span>**4.9.7 Layout design-in guideline**
...
- Is the receiver placed as recommended in the [Layout](#page-106-0) and Layout [guidance](#page-108-0)?
...
# <span id="page-113-0"></span>**5 Product handling**

# <span id="page-113-1"></span>**5.1 ESD handling precautions**
...
- Open the package within an ESD-protected area (EPA), as in [Figure](#page-114-1) 53.
...
# <span id="page-114-0"></span>**5.2 Soldering**
...
For the recommended soldering profile and conditions, see [Figure](#page-115-0) 54, [Figure](#page-116-0) 55 and [Table](#page-116-1) 43

**Figure 54: Recommended soldering profile for professional grade ZED-F9K**
...
**Figure 55: Recommended soldering profile for automotive grade ZED-F9K**
...
| Time limit above 217 °C                                   | 60 - 120 s   |                                                                                                                                                                                          |
| liquidus temperature                                      |              |                                                                                                                                                                                          |
| for automotive grade modules                              |              |                                                                                                                                                                                          |
| Peak reflow temperature<br>for professional grade modules | 245 °C       |                                                                                                                                                                                          |
...
# <span id="page-118-0"></span>**5.3 Tapes**

[Figure](#page-119-1) 56 illustrates the feed direction, component orientation, and tape dimensions (mm).The tape feeds into the pick-and-place machine from the reel on the left side of the figure and moves to the right.



<span id="page-119-1"></span>

**Figure 56: ZED-F9K Tape dimensions (mm)**

# <span id="page-119-0"></span>**5.4 Reels**

The ZED-F9K receivers are deliverable in quantities of 250 pieces on a reel.The receivers are shipped on reel type B, as specified in the Product packaging reference guide [[4](#page-126-4)].



# <span id="page-120-0"></span>**Appendix**

# <span id="page-120-1"></span>**A Glossary**
...
# <span id="page-121-0"></span>**B Reference frames**
...
# <span id="page-121-1"></span>**C RTK configuration procedures with u-center**

This section provides some guidance for using u-center to evaluate ZED-F9K RTK operation.

# <span id="page-121-2"></span>**C.1 Receiver configuration with u-center**
...
**1.** Select **Group**: CFG-UART1, **Key name**: CFG-UART1-BAUDRATE. See [Figure](#page-122-0) 57.
...
|------------------------------------------------------------------|----------------------|-------------------|-------|-------------------------------------------------------------------------------------------|
| Compose list entry<br>Group<br>CFG-UART1<br>$\blacktriangledown$ |                      | Details:<br>Title |       | The baud rate that should be configured on the<br>×.<br>UART1<br>$\overline{\phantom{a}}$ |
| Key name<br>CFG-UART1-BAUDRATE                                   | $\blacktriangledown$ | Description       |       | À.                                                                                        |
| 40520001<br>Key ID                                               | Add to List          | Type: U4          |       |                                                                                           |
...
|                                                                  |                      |                   |       |                                                                                           |
|                                                                  |                      |                   |       | Read receiver                                                                             |
| Layers<br>$\Box$ Flash $\Box$ BBR<br>$\nabla$ RAM                |                      |                   |       | Transaction<br>No transaction<br>$\blacktriangledown$                                     |

**Figure 57: u-center UBX-CFG-VALSET message view**

- **2.** Select **Add to List**, it will appear in the table below.
- **3.** Select the added key. It will now give the option of setting or reading the current value. See [Figure](#page-122-1) 58.

<span id="page-122-1"></span>

| Compose list entry<br>Group<br>Key name<br>Key ID | CFG-UART1<br>$\blacktriangledown$<br>CFG-UART1-BAUDRATE<br>40520001 | $\blacktriangledown$<br>Add to List | Details<br>Title<br>Type: U4 | Description | The baud rate that should be configured on the<br>UART1 | $\mathbb{A}$<br>$\overline{\phantom{a}}$<br>A. |
|---------------------------------------------------|---------------------------------------------------------------------|-------------------------------------|------------------------------|-------------|---------------------------------------------------------|------------------------------------------------|
|                                                   | Configuration changes to send-                                      |                                     |                              |             |                                                         |                                                |
| Key                                               | CFG-UART1-BAUDRATE                                                  | Key ID<br>0x40520001                | Type<br>U4                   | Value       |                                                         |                                                |
|                                                   |                                                                     |                                     |                              |             |                                                         |                                                |
|                                                   |                                                                     |                                     |                              |             |                                                         |                                                |
|                                                   |                                                                     |                                     |                              |             |                                                         |                                                |
|                                                   |                                                                     |                                     |                              |             |                                                         |                                                |
...
**Figure 58: Example u-center UBX-CFG-VALSET message view when selecting a configuration item**



- **4.** Add the value, for example, 230400 into the "Value" window that appears below the list. See [Figure](#page-123-0) 59.
...
| Compose list entry-<br>CFG-UART1<br>Group<br>≖ |                      | Details<br>Title | The baud rate that should be configured on the<br>luart1 | ۸<br>٠        |
|------------------------------------------------|----------------------|------------------|----------------------------------------------------------|---------------|
| Key name<br>CFG-UART1-BAUDRATE                 | $\blacktriangledown$ |                  | Description                                              | À             |
| 40520001<br>Key ID                             | Add to List          | Type: U4         |                                                          |               |
| Configuration changes to send                  |                      |                  |                                                          |               |
| Key                                            | Key ID               | Type             | Value                                                    |               |
| CFG-UART1-BAUDRATE                             | 0x40520001           | U4               | 230400 0x38400                                           |               |
|                                                |                      |                  |                                                          |               |
|                                                |                      |                  |                                                          |               |
|                                                |                      |                  |                                                          |               |
|                                                |                      |                  |                                                          |               |
...
**Figure 59: u-center UBX-CFG-VALSET message view for setting the CFG-UART1-BAUDRATE configuration item that controls the baud rate of UART1**
...
- **3.** Click **Send**. See [Figure](#page-124-0) 60.
...
| UBX - CFG (Config) - VALSET (New Configuration)                   |                      |                  |                          |                                                                                 |
|-------------------------------------------------------------------|----------------------|------------------|--------------------------|---------------------------------------------------------------------------------|
| Compose list entry<br>CFG-MSGOUT<br>Group<br>$\blacktriangledown$ |                      | Details<br>Title |                          | Output rate of the UBX-NAV-SOL message on<br>$\mathcal{O}_1$<br>port UART1<br>v |
...
| CFG-MSGOUT-UBX NAV PVT UART1                                      | 0x20910007           | U1               | $\sim$                   |                                                                                 |
| CFG-MSGOUT-UBX NAV POSLLH UART1                                   | 0x2091002a           | U1               | ÷.                       |                                                                                 |
| CFG-MSGOUT-UBX NAV RELPOSNED UART1                                | 0x2091008e           | U1               | $\overline{\phantom{a}}$ |                                                                                 |
| CFG-MSGOUT-UBX NAV STATUS UART1                                   | 0x2091001b           | U1               | ä,                       |                                                                                 |
|                                                                   |                      |                  |                          |                                                                                 |
|                                                                   |                      |                  |                          |                                                                                 |
|                                                                   |                      |                  |                          |                                                                                 |
|                                                                   |                      |                  |                          |                                                                                 |
...
**Figure 60: u-center UBX-CFG-VALSET message view for setting the CFG-MSGOUT-\* configuration items for enabling the output of some recommended UBX messages**

<span id="page-124-1"></span>**4.** To ensure all the required RTCM messages, including most importantly RTCM 1005, are being received regularly, examine the **UBX-RXM-RTCM** output in u-center. See [Figure](#page-124-1) 61.
...
| 1127              | 33                                       | 33                            | $\mathbf 0$         | $\overline{0}$              |
| 1230              | 33                                       | 33                            | 0                   | $\mathbf{0}$                |
| 1001              | 0                                        | 0                             | 0                   | n/a                         |
|                   | * Messages not supported by the receiver |                               |                     |                             |
| Current messages: |                                          |                               |                     |                             |
| ix.               | Reference Station ID                     | CRC check<br>Message ID       | ▲                   |                             |
| 198               | 0                                        | 1005<br>Passed                |                     |                             |
| 197               | $\theta$                                 | 1230 Passed                   |                     |                             |
| 196               | $\mathbf 0$                              | 1127<br>Passed                |                     |                             |
| 195               | 0                                        | 1097<br>Passed                |                     |                             |
| 194               | 0                                        | Passed<br>1087                | Ξ                   |                             |
| 193               | $\theta$                                 | 1077<br>Passed                |                     |                             |
| 192               | $\mathbf 0$                              | 1005<br>Passed                |                     |                             |
| 191               | 0                                        | 1230 Passed                   |                     |                             |
| 190               | 0                                        | 1127<br>Passed                |                     |                             |
| 189               | 0                                        | 1097<br>Passed                |                     |                             |
| 188               | 0                                        | 1087<br>Passed                |                     |                             |
| 187               | 0                                        | 1077<br>Passed                |                     |                             |
| 186               | 0                                        | 1005<br>Passed                |                     |                             |
| 185<br>184        | 0<br>0                                   | 1230 Passed<br>1127<br>Passed |                     |                             |

**Figure 61: u-center UBX-RXM-RTCM view**



<span id="page-125-0"></span>**5.** Once the receiver is receiving a valid set of RTCM messages, it should transition through 3D Fix to 3D/DGNSS to Float and, ultimately, into Fixed mode. See [Figure](#page-125-0) 62.
...
**Figure 62: u-center data view with RTK Fixed**
...
- <span id="page-126-3"></span>**[1]** ZED-F9K-00B Data sheet, UBX-20032561 ZED-F9K-01A Data sheet, UBX-21045820 ZED-F9K-02A Data sheet, UBXDOC-304424225-18291
- <span id="page-126-2"></span>**[2]** LAP 1.20 Interface description, UBX-20046191 LAP 1.30 Interface description, UBX-22005157 LAP 1.50 Interface description, UBXDOC-963802114-13052
- <span id="page-126-1"></span>**[3]** u-blox F9 LAP 1.50 Release notes, UBXDOC-963802114-13007
- <span id="page-126-4"></span>**[4]** Product packaging reference guide, [UBX-14001652](https://www.u-blox.com/docs/UBX-14001652)
...
|----------|-------------|------------------------------------------------------------------------------------------------------------------------------|--|--|--|--|--|
| R01      | 06-Nov-2020 | Early production information - ZED-F9K-00B-01                                                                                |  |  |  |  |  |
| R02      | 24-Aug-2021 | Advance information - ZED-F9K-00B-02                                                                                         |  |  |  |  |  |
| R03      | 03-May-2023 | Advance information - ZED-F9K-01A-55                                                                                         |  |  |  |  |  |
|          |             | Changes:                                                                                                                     |  |  |  |  |  |
|          |             | - Switching between L1 L2 and L1 L5                                                                                          |  |  |  |  |  |
|          |             | - SPARTN version 2.0.1 protocol description added                                                                            |  |  |  |  |  |
|          |             | - UBX protocol enabled with UART 2                                                                                           |  |  |  |  |  |
|          |             | - Interrupt-based IMU sensor output description added                                                                        |  |  |  |  |  |
|          |             | - UBX-NAV-PVAT supported                                                                                                     |  |  |  |  |  |
|          |             | - Wake on motion description added                                                                                           |  |  |  |  |  |
|          |             | - Secondary GNSS output description added                                                                                    |  |  |  |  |  |
|          |             | - Protection level description added                                                                                         |  |  |  |  |  |
|          |             | - Advanced calibration handling description added                                                                            |  |  |  |  |  |
|          |             | - Sensor based spoofing detection description added                                                                          |  |  |  |  |  |
|          |             | - Spectrum analyzer description added                                                                                        |  |  |  |  |  |
|          |             | - Updated I2C and SPI timing specifications in section Communications interfaces                                             |  |  |  |  |  |
| R04      | 24-Jul-2023 | Initial production - ZED-F9K-01A                                                                                             |  |  |  |  |  |
| R05      | 04-Mar-2025 | Added ZED-F9K-02A-00 variant                                                                                                 |  |  |  |  |  |
|          |             | Firmware version to LAP 1.50                                                                                                 |  |  |  |  |  |
|          |             | Added sections:                                                                                                              |  |  |  |  |  |
|          |             | - CLAS corrections                                                                                                           |  |  |  |  |  |
|          |             | - Motorbike dynamic platform model                                                                                           |  |  |  |  |  |
|          |             | - SBAS and BDSBAS configuration                                                                                              |  |  |  |  |  |
|          |             | -<br>New<br>configuration:<br>CFG-NAVHPG-DGNSSMODE=RTK_CAR,<br>CFG-SFIMU<br>IMU_MNTALG_TOLERANCE and CFG-MOT-IMU_FILT_WINDOW |  |  |  |  |  |
|          |             | Updated sections:                                                                                                            |  |  |  |  |  |
|          |             | - Default GNSS configuration                                                                                                 |  |  |  |  |  |
|          |             | - SPARTN2.0.2 and RTCM3.4 protocols                                                                                          |  |  |  |  |  |
| R06      | 04-Apr-2025 | Initial production - ZED-F9K-02A                                                                                             |  |  |  |  |  |
|          |             |                                                                                                                              |  |  |  |  |  |
